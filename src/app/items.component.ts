...
import {MdSelect, MdOption, MdSnackBar} from '@angular/material';
  ...
  ngOnInit() {
    Observable.fromEvent(this.element.nativeElement, 'keydown', true).subscribe((x: KeyboardEvent) => {
      if (x.key === 'Enter') {
        ...
        if (option) this.mdSelect.writeValue(option.value);
        this.select((<MdOption>this.mdSelect.selected).value);
      }
    });
